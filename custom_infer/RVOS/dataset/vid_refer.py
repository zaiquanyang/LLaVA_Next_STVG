...
class Vid_REFER:
    ...
    def __init__(self, data_root='/home/yzq/mnt/code/RIS/segmentation/MeViS/RVOS_Data', dataset='refcoco', split='train'):
        ...
        if dataset in ['mevis', 'refer_youtube_vos']:
            self.IMAGE_DIR = osp.join(data_root, dataset, 'JPEGImages')
        else:
        ...
        tic = time.time()
        ref_file = osp.join(self.DATA_DIR, 'vid_refs.p')
    ...
    def getRefIds(self, image_ids=[], cat_ids=[], ref_ids=[], split=''):
        ...
        if len(image_ids)==len(cat_ids)==len(ref_ids)==len(split)==0:
            ...
            if not len(split) == 0:
                ...
                elif split == 'valid_u':            # for video_mevis
                    refs = [ref for ref in refs if ref['split'] == split]
                else:
