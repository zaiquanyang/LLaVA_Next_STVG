...
from custom_infer.RVOS.dataset.vid_dataset import Vid_ReferDataset
from custom_infer.RVOS.dataset.transform import get_vid_transform, get_img_transform
...
def run_inference(args):
    ...
    for vid_data in vid_dataset:
        samples, targets = vid_data
        ref_sent = targets['sentences'][0]
        vid_path = targets['vid_path']

        
        question = f"Is there a {ref_sent} in this image or video ?"
        ...
        os.makedirs(f'custom_infer/RVOS/output/{vid_name}', exist_ok=True)
        ...
        vid_rela.save(f'custom_infer/RVOS/output/{vid_name}/{question}_rela_att_norm_true.png')
