...
from custom_infer.vis import show_image_relevance, show_video_relevance, norm_video_att, original_resize_336, blur_proposal_video_array
...
def llava_embedd_tuning(cfg_pretrained, tokenizer, model, image_processor, video, questions, grid_size, targets, args, vid_i, logger):
    ...
    if llm_reasoning:
        ...
        if args.ttt_epoch > 0:
            tmp_questions = prepare_instructions(questions, targets, mode='whole_tuning')
            logger.info(f'whole_tuning:  {tmp_questions}')
        ...
        tmp_questions = prepare_instructions(questions, targets, mode='whole_infer')
        logger.info(f'whole_infer:  {tmp_questions}')
