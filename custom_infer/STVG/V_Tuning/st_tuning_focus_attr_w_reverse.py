...
from custom_infer.STVG.V_Tuning.spatial_tuning import spatial_llm_logit, spatial_llm_att, spatial_llm_reverse_logit, spatial_llm_att_w_reverse, spatial_llm_att_twice
...
def llava_embedd_tuning(cfg_pretrained, tokenizer, model, image_processor, video, questions, grid_size, targets, args, vid_i, logger):
    ...
    if llm_reasoning:
        ...
        spatial_prompt_pseudo = torch.nn.Parameter(torch.zeros_like(img_token_embedds)) # vp_tuning
        # spatial_prompt = torch.nn.Parameter(torch.zeros((1, 6, 4096)).to(img_token_embedds))             # tp_tuning
        spatial_prompt_history_pseudo = spatial_prompt_pseudo
        ...
        if args.ttt_epoch > 0:
            ...
            spatial_prompt, spatial_prompt_history = spatial_llm_logit(
                                                    ...
                                                    img_token_embedds, 
                                                    None,
                                                    spatial_prompt_pseudo, spatial_prompt_history_pseudo, 
                                                    grid_size, 
                                                    video_batch_wo_blur, 
                                                    tokens_format, 
                                                    args.ttt_epoch,
            ...
            spatial_prompt_reverse, spatial_prompt_history_reverse = spatial_llm_logit(
                                                    model, 
                                                    tmp_questions.copy(), 
                                                    pre_img_token_embedds, 
        ...
        llava_att_pred_track_id, llava_rela_att = spatial_llm_att_twice(
                                                model,
                                                tmp_questions.copy(),
                                                pre_img_token_embedds,
                                                img_token_embedds,
        ...
        name_f = targets['vid_name']
        # ============== load spatial_pred_track_id
        npy_f = f'custom_infer/STVG/output/hcstvg2/llava_onevision_qwen2_7b_ov_f_20/focus_attr_ep_2_lr_8.0_8.0_token_-3_w_aug/_llava_att_npy/{name_f}.npy'
        llava_dict = np.load(npy_f, allow_pickle=True).item()
        spatial_pred_track_id = llava_dict['S_pred']
        track_id = spatial_pred_track_id
        spatial_pred = {'Space_pred': track_id}
        
        # ============== load llava_rela_att
        npy_f = f'custom_infer/STVG/output/hcstvg2/llava_onevision_qwen2_7b_ov_f_20/focus_act_ep_2_lr_8.0_8.0_token_-3_(act_cap_ref_cap)/_llava_att_npy/{name_f}.npy'
        llava_dict = np.load(npy_f, allow_pickle=True).item()
