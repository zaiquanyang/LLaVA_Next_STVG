...
def llava_embedd_tuning(cfg_pretrained, tokenizer, model, image_processor, video, questions, grid_size, targets, args, vid_i, logger):
    ...
    if not os.path.exists(npy_f):
        ...
        spatial_prompt_, spatial_prompt_history_ = spatial_llm_logit_joint_tuning(
                                                        model, 
                                                        tmp_questions.copy(), 
                                                        pre_img_token_embedds, 
                                                        img_token_embedds_wo_blur, 
                                                        inputs_blur_track_embeds,
                                                        spatial_prompt, spatial_prompt_history, temporal_prompt, temporal_prompt_history,
                                                        grid_size, 
                                                        video_batch_wo_blur, 
                                                        tokens_format, 
                                                        args.ttt_epoch,
        ...
        bbox_pred_tmp, temporal_pred_tmp = ttt_eval(targets, llava_rela_att, vid_i, preprocess, contrastive_content, spatial_pred, args, save=True)
        print('Evaluate and Visualize the llava_att on spatial prediction done!')
        
        # # 保存 llava_rela_att 到指定文件夹
        # npy_dict = {}
        # npy_dict = {'S_pred': spatial_pred_track_id, 'T_LLaVa_att': llava_rela_att.cpu().numpy()}
        # name_f = targets['vid_name']
        ...
        spatial_pred = {'Space_pred': spatial_pred_track_id}
        contrastive_content  = f'track_{spatial_pred_track_id}_{questions[0:120]}_'
        bbox_pred_tmp, temporal_pred_tmp = ttt_eval(targets, llava_rela_att, vid_i, preprocess, contrastive_content, spatial_pred, args, save=False)
        print('Evaluate and Visualize the llava_att on spatial prediction done!')
    # breakpoint()
