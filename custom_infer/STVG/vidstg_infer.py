...
from custom_infer.utils import load_video, load_video_base64, Q_to_prompt, torch_infer, load_img_list
...
def run_inference(args):
    ...
    cfg.merge_from_file('custom_infer/STVG/experiments/vidstg.yaml')
    # cfg.merge_from_list(args.opts)
    cfg.freeze()

    # split = "train"
    # dataset = 'mevis'
    # vid_dataset = Vid_ReferDataset(refer_data_root="/home/yzq/mnt/code/RIS/segmentation/Weak_RVOS/TRIS/RVOS_Data",
    #                         dataset=dataset,
    #                         splitBy="none",
    #                         bert_tokenizer="clip",
    #                         split=split,
    ...
    #                         size=384,
    #                         max_tokens=20,
    #                         image_transforms=get_vid_transform(size=384, train_mode=False),
    #                         eval_mode=True,
    #                         scales=None,
    #                         positive_samples=0)

    vid_dataset = build_dataset(cfg, split='test', transforms=None)

    kk = 0
    for i in range(len(vid_dataset)):
        kk += 1
        try:
            vid_data = vid_dataset[i]
        except Exception as e:
            print(f"Error occurred at sample {i}: {e}")
            # import traceback
        ...
        class_catgory = targets['class_catgory']
        ...
        if video_time > 30 or qtype == 'inter':
            continue
        # print(kk, '/', len(vid_dataset), vid_path)
        # continue
        
        question = f"Is there {ref_sent} in this image or video?"
        cap = 'please describe this video/image'

        # ============= Check if the video exists
        # if os.path.exists(vid_path):
        #     video_, frame_time, video_time, pil_images = load_img_list(vid_path, frames_N=20)
        #     video = image_processor.preprocess(video_, return_tensors="pt")["pixel_values"].half().cuda()
        #     video = [video]
        # else:
        # breakpoint()
        video = image_processor.preprocess(video_data, return_tensors="pt")["pixel_values"].half().cuda()
        video = [video]
        ...
        vid_name = vid_path
        os.makedirs(f'custom_infer/STVG/output/', exist_ok=True)
        ...
        class_catgory_path = class_catgory.replace('/', '_')
        vid_rela.save(f'custom_infer/STVG/output/{class_catgory_path}_{vid_path}_{question}_rela_att_norm.png')
        # breakpoint()
